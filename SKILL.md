...
description: 分析用户提供的单个文件（可以是文件路径，也可以是直接粘贴的文件内容），从安全测试角度逐行观察，输出一个 JSON 列表，列出所有安全相关的观察点，每个点带预定义标签、精确行号和原文片段。当用户要求审查/扫描/检查/审计某一个具体文件的安全问题时使用本技能——例如"分析 /app/src/auth.py 的安全问题""检查这个配置文件有没有安全隐患""这个文件里有哪些安全关注点""对 X 做一次安全审查""看看这个文件的安全情况""分析下面这段代码的安全关注点"。只报告能直接看到的事实，绝不推测风险、严重程度或可利用性。适用于任何语言、任何类型的单个文件。不搜索文件系统，也不用 grep/glob 等搜索工具去匹配关键词——而是直接读入整个文件、逐行分析，文件很大时也通读。严格只处理用户给的那一个文件（路径或内容），不打开或搜索其他文件。
---
