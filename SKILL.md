...
- `points[].snippet`：原文片段，逐字照抄，让测试人员能找到它。跨多行时取完整片段，长行用 `…` 截断。
...
5. 统计标签，组装 JSON。
6. 调用脚本记录结果。将 JSON 压缩为一行（JSONL 格式），通过 stdin 传入：
   ```bash
   echo '<单行json>' | python <skill_dir>/scripts/collect_result.py
   ```
   脚本自动创建 `.secscan/` 目录并追加到 `.secscan/results.jsonl`，每行一个 JSON 对象。
   - 必须确保每个文件的结果是**单行 JSON**（不含内部换行）。
   - 支持传入多行，每行一个文件的结果。
   - `content` 模式（`file: "inline"`）也会写入。
   - 多次扫描同一文件会产生多行记录（均保留）。
