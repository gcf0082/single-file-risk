...
多个文件时，逐个文件走完整流程（步骤 1-7），处理完一个再处理下一个。
...
5. 统计标签，组装 JSON，按"输出与记录"章节调用脚本记录结果。
6. 输出 JSON 给用户。
7. 如果观察不到任何安全相关的东西，仍输出 JSON，`"points": []`，并加一个顶层 `"note": "未观察到安全相关内容"`。
